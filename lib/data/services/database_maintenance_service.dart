...
final String _dbUploadApiUrl = AppConfig.baseUrl;
    ...
    try {
      ...
      if (fileBytes.isEmpty) {
        return const SyncError(
            message: 'Database file is empty or could not be read.');
      }
      ...
      final String uniqueFileName =
          'biochecksheet_backup_${finalDeviceId}_${DateTime.now().millisecondsSinceEpoch}.zip';
      ...
      final uri = Uri.parse('$_dbUploadApiUrl/api/database/upload');
      print('Database upload uri: $uri');

      final request = http.Request('POST', uri) // <<< Use http.Request
        ..headers['Content-Type'] =
            'application/zip' // <<< Set Content-Type for the entire body
        ..headers['userId'] = userId ?? 'unknown' // <<< Add userId to header
        ..headers['deviceId'] =
            deviceId ?? 'unknown' // <<< Add deviceId to header
        ..bodyBytes = zipBytes; // <<< Set the zip file bytes as the body

      final response =
          await http.Client().send(request); // <<< Send the Request
      ...
      return SyncError(
          exception: e, message: 'ข้อผิดพลาดในการสำรองและอัปโหลดฐานข้อมูล: $e');
    } finally {
