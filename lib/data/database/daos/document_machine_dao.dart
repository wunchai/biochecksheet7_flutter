...
class DocumentMachineDao extends DatabaseAccessor<AppDatabase> with _$DocumentMachineDaoMixin {
  ...
  Future<int> insertDocumentMachine(DocumentMachinesCompanion entry) => into(documentMachines).insert(entry);

  // Equivalent to suspend fun insertAll(documentMachines: List<DbDocumentMachine>)
  Future<void> insertAllDocumentMachines(List<DocumentMachinesCompanion> entries) async {
    await batch((batch) {
  ...
  Future<DbDocumentMachine?> getDocumentMachine(String documentId, String machineId) {
  ...
  Stream<List<DbDocumentMachine>> watchAllDocumentMachines() => select(documentMachines).watch();
  Future<List<DbDocumentMachine>> getAllDocumentMachines() => select(documentMachines).get();
  ...
  Future<List<DbDocumentMachine>> getDocumentMachineList(String documentId) {
    return (select(documentMachines)..where((tbl) => tbl.documentId.equals(documentId))).get();
  }

  // Equivalent to suspend fun updateDocumentMachine(documentMachine: DbDocumentMachine)
  Future<bool> updateDocumentMachine(DbDocumentMachine entry) => update(documentMachines).replace(entry);

  // Equivalent to suspend fun deleteDocumentMachine(documentMachine: DbDocumentMachine)
  Future<int> deleteDocumentMachine(DbDocumentMachine entry) => delete(documentMachines).delete(entry);
