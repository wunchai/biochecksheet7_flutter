...
class JobMachineDao extends DatabaseAccessor<AppDatabase> with _$JobMachineDaoMixin {
  ...
  Future<int> insertJobMachine(JobMachinesCompanion entry) => into(jobMachines).insert(entry);
  ...
  Stream<List<DbJobMachine>> watchAllJobMachines() => select(jobMachines).watch();
  ...
  Future<bool> updateJobMachine(DbJobMachine entry) => update(jobMachines).replace(entry);

  // Equivalent to suspend fun deleteJobMachine(jobMachine: DbJobMachine)
  Future<int> deleteJobMachine(DbJobMachine entry) => delete(jobMachines).delete(entry);
