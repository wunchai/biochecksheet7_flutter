...
class ProblemApiService {
    ...
    try {
      final response = await http
          ...
          .timeout(const Duration(
              seconds:
                  15)); // <<< CRUCIAL FIX: กำหนด Timeout ที่นี่ (15 วินาที)
      ...
      if (response.statusCode == 200) {
          ...
          final List<DbProblem> syncedProblems = problemList.map((problemData) {
            ...
            return DbProblem(
              ...
              problemSolvingDescription:
                  problemData['problemSolvingDescription'],
              documentId: problemData['documentId']
                  ?.toString(), // <<< NEW: Map documentId
              machineId: problemData['machineId']
                  ?.toString(), // <<< NEW: Map machineId
              machineName:
                  problemData['machineName'], // <<< NEW: Map machineName
              ...
              description: problemData[
                  'description'], // <<< Corrected: Use TagDescription
              ...
              syncStatus:
                  int.tryParse(problemData['syncStatus']?.toString() ?? '0') ??
                      0,
            );
          }).toList();
        ...
        throw Exception(
            "Problem Sync failed: Status code ${response.statusCode}");
      }
    } on TimeoutException {
      // <<< CRUCIAL FIX: Re-throw TimeoutException
      print("Network timeout during problem sync.");
      rethrow; // Re-throw the original TimeoutException
    } on http.ClientException catch (e) {
      // <<< CRUCIAL FIX: Re-throw ClientException
  ...
  Future<List<UploadRecordResult>> uploadProblems(
      List<DbProblem> problemsToUpload) async {
    // <<< Corrected Return Type
    final uri = Uri.parse(
        "$_baseUrl/CHECKSHEET_PROBLEMRECORD_SYNC"); // Assumed API Endpoint
    print("Uploading problems to API: $uri");
    final headers = {"Content-Type": "application/json"};

    final List<Map<String, dynamic>> jsonProblems =
        problemsToUpload.map((problem) {
      return {
        "uid": problem.uid,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['Table1'] != null && responseJson['Table1'] is List) {
          ...
          return resultsList
              ...
              .toList(); // Map to UploadRecordResult
        } else {
          throw Exception(
              "Problem Upload API response format invalid (missing 'Table1' key or not a list).");
        }
      } else {
        throw Exception(
            "Problem Upload API failed: Status code ${response.statusCode}");
      }
    } on http.ClientException catch (e) {
