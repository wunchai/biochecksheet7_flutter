...
class DocumentViewModel extends ChangeNotifier {
  ...
  Future<void> loadDocuments(String? jobId, {String? searchQuery}) async {
    ...
    try {
      ...
      if (jobId != null && jobId.isNotEmpty) {
        baseQuery = _documentDao.select(_documentDao.documents)..where((tbl) => tbl.jobId.equals(jobId));
      } else {
        baseQuery = _documentDao.select(_documentDao.documents);
      }

      if (searchQuery != null && searchQuery.isNotEmpty) {
        baseQuery = baseQuery..where((tbl) => tbl.documentName.contains(searchQuery) | tbl.documentId.contains(searchQuery));
      }
  ...
  Future<bool> deleteSelectedDocument() async {
    ...
    try {
      ...
      _syncMessage = "Document '${_selectedDocument!.documentName}' deleted successfully!";
