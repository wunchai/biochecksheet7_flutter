...
class DocumentRecordAppBar extends StatefulWidget implements PreferredSizeWidget {
  final String title;
...
class _DocumentRecordAppBarState extends State<DocumentRecordAppBar> {
  ...
  bool _canPostButtonEnabled = false; // Internal state for the Post button's enabled status.
  bool _canUploadButtonEnabled = false; // Internal state for the Upload button's enabled status.

  StreamSubscription<List<DocumentRecordWithTagAndProblem>>? _recordsSubscription;
  ...
  Future<void> _updateButtonStates(List<DocumentRecordWithTagAndProblem>? records, bool viewModelIsLoading) async {
    List<DocumentRecordWithTagAndProblem> currentRecords = records ?? [];
    if (records == null) {
      currentRecords = await (_viewModel.recordsStream?.first ?? Future.value([]));
    }

    bool newCanPost = true;
    bool newCanUpload = false; // Start as false, set to true only if conditions are met
    ...
    if (currentRecords.isEmpty) {
      ...
      for (var r in currentRecords) {
        print('  UID: ${r.documentRecord.uid}, TagName: ${r.jobTag?.tagName}, Status: ${r.documentRecord.status}');
        if (r.documentRecord.status != 1) {
          allAreStatus1 = false;
        } else {
          anyAreStatus1 = true; // At least one is status 1
        }
        if (r.documentRecord.status != 2) { // Check if not status 2 for 'allAreStatus2'
          allAreStatus2 = false;
        } else {
      ...
      print('Summary: allAreStatus1=$allAreStatus1, anyAreStatus0=$anyAreStatus0, allAreStatus2=$allAreStatus2, anyAreStatus2(old)=$anyAreStatus2'); // Corrected anyAreStatus2 logging

      // Logic for Post button (unchanged logic, only name changed for clarity)
      if (allAreStatus1 && !anyAreStatus2) { // All Status 1, and NONE are Status 2
        newCanPost = true;
      ...
      if (currentRecords.isNotEmpty && !anyAreStatus0) { // If there are records AND NONE are status 0
          if (allAreStatus1 || allAreStatus2) { // If ALL are status 1 OR ALL are status 2
              newCanUpload = true;
              print('Upload button: ENABLED (All Status 1 OR All Status 2)');
          } else {
              newCanUpload = false;
              print('Upload button: DISABLED (Records are mixed, e.g., some 1, some 2, but not ALL 1 or ALL 2)');
          }
      } else { // No records OR some records are status 0
          newCanUpload = false;
          print('Upload button: DISABLED (No records OR some records are Status 0)');
      }
    }
  ...
  Widget build(BuildContext context) {
    final viewModel = Provider.of<DocumentRecordViewModel>(context); // Listen to ViewModel changes.
