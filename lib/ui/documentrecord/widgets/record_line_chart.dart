    ...
    return StreamBuilder<List<FlSpot>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Center(child: Text('ข้อผิดพลาดในการโหลดกราฟ: ${snapshot.error}'));
          ...
          minY = (minY * 0.95).clamp(0.0, double.infinity); // 5% padding below min
          maxY = maxY * 1.05; // 5% padding above max

          // Ensure minY is always less than maxY
          if (minY >= maxY) {
            minY = maxY - 1.0; // Prevent invalid chart range if all values are same
            if (minY < 0) minY = 0.0;
          }
          ...
          if (specMin != null) {
            lineBarsData.add(
              LineChartBarData(
                spots: [
                  FlSpot(0, specMin),
                  FlSpot((spots.length - 1).toDouble().clamp(0.0, double.infinity), specMin),
                ],
          ...
          if (specMax != null) {
            lineBarsData.add(
              LineChartBarData(
                spots: [
                  FlSpot(0, specMax),
                  FlSpot((spots.length - 1).toDouble().clamp(0.0, double.infinity), specMax),
                ],
                isCurved: false, // Straight line
                ...
                titlesData: FlTitlesData(
                  ...
                  leftTitles: AxisTitles(
                    sideTitles: SideTitles(
                      showTitles: true,
                      reservedSize: 40,
                      getTitlesWidget: (value, meta) => Text(value.toStringAsFixed(0)), // Y-axis labels
                    ),
                  ),
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      showTitles: true,
                      reservedSize: 40,
                      getTitlesWidget: (value, meta) => Text(value.toInt().toString()), // X-axis labels (index)
                    ),
                  ),
