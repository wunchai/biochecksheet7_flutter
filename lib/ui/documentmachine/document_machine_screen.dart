...
class _DocumentMachineScreenState extends State<DocumentMachineScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<DocumentMachineViewModel>(context, listen: false).loadMachines(widget.documentId, widget.jobId);
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              Provider.of<DocumentMachineViewModel>(context, listen: false).refreshMachines(); // เรียก refresh ใน ViewModel
            },
          ),
                      ...
                      builder: (context, snapshot) {
                        if (viewModel.isLoading && !snapshot.hasData) {
                          ...
                          return ListView.builder(
                            itemCount: machines.length, // จำนวนเครื่องจักรในรายการ
                            padding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 4.0), // Padding สำหรับทั้งรายการ
                            itemBuilder: (context, index) {
                              final machine = machines[index]; // ได้รับรายการเครื่องจักรปัจจุบัน
                              ...
                              return Card(
                                ...
                                child: InkWell(
                                  ...
                                  onTap: () {
                                    ...
                                    print('แตะเครื่องจักร: ${machine.machineName}');
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => DocumentRecordScreen(
                                          title: 'บันทึก: ${machine.machineName ?? 'N/A'}', // Title สำหรับหน้า Record
                                          documentId: widget.documentId, // ส่ง documentId ปัจจุบัน
                                          machineId: machine.machineId ?? '',
                                          jobId: viewModel.jobId ?? '', // <<< ส่ง jobId ไปด้วย // ส่ง machineId ของเครื่องจักรที่แตะ
                                        ),
                                      ),
                                    );
                                  },
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          machine.machineName ?? 'N/A', // แสดงชื่อเครื่องจักร
                                        ...
                                        Text('รหัสเครื่องจักร: ${machine.machineId ?? 'N/A'}', style: Theme.of(context).textTheme.bodySmall), // แสดงรหัสเครื่องจักร
                                        Text('ประเภท: ${machine.machineType ?? 'N/A'}', style: Theme.of(context).textTheme.bodySmall), // แสดงประเภท
                                        Text('คำอธิบาย: ${machine.description ?? 'N/A'}', style: Theme.of(context).textTheme.bodySmall), // แสดงคำอธิบาย
                                        Text('ข้อมูลจำเพาะ: ${machine.specification ?? 'N/A'}', style: Theme.of(context).textTheme.bodySmall), // แสดงข้อมูลจำเพาะ
                                        Text('สถานะ: ${machine.status ?? 'N/A'}', style: Theme.of(context).textTheme.bodySmall), // แสดงสถานะ
                                      ],
                                    ),
                                  ),
                                ),
