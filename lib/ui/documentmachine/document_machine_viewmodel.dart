...
class DocumentMachineViewModel extends ChangeNotifier {
  ...
  Future<void> loadMachines(String? documentId, String? jobId) async {
    ...
    try {
      ...
      drift.SimpleSelectStatement<$DocumentMachinesTable, DbDocumentMachine> baseQuery;

      if (documentId != null && documentId.isNotEmpty && jobId != null && jobId.isNotEmpty) {
        // Use the DAO's instance of select and where
       print("if statement : $documentId and Job ID: $jobId");

        baseQuery = _documentMachineDao.select(_documentMachineDao.db.documentMachines) // Access table via db instance
            ..where((tbl) => tbl.jobId.equals(jobId));
           
        _statusMessage = "Machines for Document ID: $documentId (Job ID: $jobId) loaded.";
