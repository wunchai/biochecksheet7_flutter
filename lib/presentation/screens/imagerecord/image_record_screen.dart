...
import 'package:biochecksheet7_flutter/presentation/screens/imagerecord/widgets/image_viewer_dialog.dart'; // NEW: Import ImageViewerDialog
...
class _ImageRecordScreenState extends State<ImageRecordScreen> {
  ...
  void initState() {
    ...
    print(
        'ImageRecordScreen: initState called. DocID=${widget.documentId}, MachineID=${widget.machineId}, JobID=${widget.jobId}, TagID=${widget.tagId}, ProblemID=${widget.problemId}'); // <<< Debugging
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(
                Icons.photo_library), // Icon for picking from gallery
            onPressed: widget.isReadOnly
                ? null
                : () {
                    final viewModel =
      ...
      body: Consumer<ImageViewModel>(
        builder: (context, viewModel, child) {
          ...
          if (viewModel.syncMessage != null && !_isShowingDialog) {
            ...
            print(
                'ImageRecordScreen: 1.currentSyncMessage (outside callback) is $currentSyncMessage'); // Debugging
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) async {
              ...
              if (mounted) {
                print(
                    'ImageRecordScreen: 2.currentSyncMessage (inside callback) is $currentSyncMessage'); // Debugging
                    ...
                    child: StreamBuilder<List<DbImage>>(
                      ...
                      builder: (context, snapshot) {
                        if (viewModel.isLoading && !snapshot.hasData) {
                          ...
                          return const Center(
                              child: Text('ไม่พบรูปภาพสำหรับ Record นี้.'));
                        } else {
                          ...
                          return GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              String?
                                  viewerImagePath; // Path to pass to ImageViewerDialog
                              ...
                              if (kIsWeb &&
                                ...
                                imageProvider =
                                    MemoryImage(base64Decode(image.picture!));
                                viewerImagePath =
                                    'data:image/jpeg;base64,${image.picture!}'; // Pass data URI for viewer
                              } else if (!kIsWeb &&
                                  image.filepath != null &&
                                ...
                                viewerImagePath = image
                                    .filepath!; // Pass file path for viewer
                              } else if (image.imageUri != null &&
                                  image.imageUri!.startsWith('data:image')) {
                                // Fallback for data URI (if stored in imageUri)
                                try {
                                  final uriBytes = base64Decode(
                                      image.imageUri!.split(',').last);
                                  imageProvider = MemoryImage(uriBytes);
                                  viewerImagePath = image
                                      .imageUri!; // Pass data URI for viewer
                                } catch (e) {
                                  print(
                                      'Error decoding data URI from imageUri for display: $e');
                                }
                              }
                              ...
                              if (imageProvider != null) {
                                  ...
                                  child: InkWell(
                                    onTap: () {
                                      ...
                                      if (kIsWeb && image.picture != null) {
                                        ...
                                        print(
                                            'Error: No valid viewerImagePath for UID ${image.uid}'); // <<< Debug
                                        ScaffoldMessenger.of(context)
                                            .showSnackBar(
                                          const SnackBar(
                                              content: Text(
                                                  'ไม่สามารถแสดงรูปภาพได้: Path ไม่ถูกต้อง.')),
                                        );
                                      }
                                    },
                                    child: Stack(
                                      ...
                                      children: [
                                        Image(
                                          // Use Image widget with imageProvider
                                          image: imageProvider,
                                          fit: BoxFit.cover,
                                          errorBuilder:
                                ...
                                return const Card(
                                  child: Center(
                                      child: Text(
                                          'ไม่มีรูปภาพ หรือ Path ไม่ถูกต้อง')),
                                );
                              }
                            },
                          );
