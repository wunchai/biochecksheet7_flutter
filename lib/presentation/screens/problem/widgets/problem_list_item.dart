...
import 'package:biochecksheet7_flutter/presentation/screens/problem/problem_viewmodel.dart'; // For ProblemViewModel
...
class ProblemListItem extends StatefulWidget {
  ...
  final VoidCallback
      onTap; // Callback when the item is tapped (to show details)
  final VoidCallback
      onShowOnlineChart; // Callback to show online chart for this problem's tag
...
class _ProblemListItemState extends State<ProblemListItem> {
  late TextEditingController
      _internalSolvingDescController; // <<< NEW: Internal controller

  @override
  void initState() {
    super.initState();
    // Initialize controller with the problem's solving description
    _internalSolvingDescController = TextEditingController(
        text: widget.problem.problemSolvingDescription ?? '');
  }
  ...
  void didUpdateWidget(covariant ProblemListItem oldWidget) {
    ...
    if (_internalSolvingDescController.text !=
        (widget.problem.problemSolvingDescription ?? '')) {
      _internalSolvingDescController.text =
          widget.problem.problemSolvingDescription ?? '';
      // Keep cursor at the end
      _internalSolvingDescController.selection = TextSelection.fromPosition(
          TextPosition(offset: _internalSolvingDescController.text.length));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isProblemReadOnly =
        widget.problem.problemStatus == 1 || widget.problem.problemStatus == 2;
    // Get ViewModel's loading status
    final bool viewModelIsLoading =
        widget.viewModel.isLoading; // Access isLoading from injected ViewModel
        ...
        child: Padding(
          ...
          child: Column(
             ...
             */
              ...
              TextField(
                controller:
                    _internalSolvingDescController, // <<< Use internal controller
                ...
                decoration: InputDecoration(
                  ...
                  hintText:
                      isReadOnly ? 'ไม่สามารถแก้ไขได้' : 'ป้อนคำอธิบายการแก้ไข',
                  border: const OutlineInputBorder(),
                  errorText: widget.viewModel.problemErrors[
                      widget.problem.uid], // Display validation error
                  suffixIcon: IconButton(
                    icon: const Icon(Icons.check),
                    onPressed: isEnabled
                        ? () {
                            // Only enable button if TextField is enabled
                            widget.viewModel.updateProblemSolvingDescription(
                                widget.problem.uid,
                                _internalSolvingDescController.text);
                          }
                        : null,
              ...
              Row(
                ...
                children: [
                  // Post button
                  TextButton(
                    onPressed: (viewModelIsLoading ||
                            isProblemReadOnly ||
                            widget.problem.problemStatus != 0)
                        ? null
                        : () {
                  ...
                  if (widget.problem.tagType == 'Number')
                    IconButton(
                      ...
                      onPressed: () {
                        widget
                            .onShowOnlineChart(); // Call callback to show chart
                      },
                    ),
                ],
              ),
