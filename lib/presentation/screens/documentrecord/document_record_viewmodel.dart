...
class DocumentRecordViewModel extends ChangeNotifier {
  ...
  final LoginRepository
      _loginRepository; // ใช้เข้าถึงข้อมูลผู้ใช้ที่ล็อกอินอยู่
  ...
  Stream<List<DocumentRecordWithTagAndProblem>>?
      _recordsStream; // Stream ของบันทึกที่จะแสดงผล
  ...
  Stream<List<FlSpot>>?
      _onlineChartDataStream; // Stream สำหรับข้อมูลกราฟ Online
  ...
  String?
      _syncMessage; // ข้อความสำหรับสถานะการซิงค์ (เช่น สำเร็จ/ล้มเหลวใน SnackBar)
  ...
  final Map<int, String?> _recordErrors =
      {}; // Map เก็บข้อผิดพลาดการ Validate ของแต่ละ Record (UID เป็น Key)
  Map<int, String?> get recordErrors => _recordErrors;

  DbDocumentRecord?
      _selectedDocument; // บันทึกที่ถูกเลือกในปัจจุบัน (สำหรับ Copy/Delete)
  ...
  Future<void> loadRecords(
    ...
    try {
      print(
          'DocumentRecordViewModel: loadRecords ถูกเรียกด้วย DocID=$documentId, MachineID=$machineId, JobID=$jobId');
      ...
      print(
          'DocumentRecordViewModel: บันทึกถูก Initialize แล้ว. กำลังโหลดจาก DB.');
      ...
      _recordsStream?.listen((data) {
        ...
        if (data.isEmpty) {
          ...
          for (var recordWithTag in data) {
            print(
                "  บันทึก: UID=${recordWithTag.documentRecord.uid}, TagName=${recordWithTag.jobTag?.tagName}, ค่า=${recordWithTag.documentRecord.value}, ประเภท Tag=${recordWithTag.jobTag?.tagType}");
          }
        }
      }, onError: (error) {
      ...
      _statusMessage =
          "บันทึกสำหรับ Document ID: $documentId, Machine ID: $machineId โหลดแล้ว.";
    } catch (e) {
  ...
  Future<void> refreshRecords() async {
    ...
    try {
      if (_documentId != null && _machineId != null && _jobId != null) {
        ...
        await loadRecords(
            _documentId!, _machineId!, _jobId!); // โหลดใหม่จาก Local DB
      } else {
        _statusMessage =
            "ไม่สามารถ Refresh ได้: documentId, machineId หรือ jobId หายไป.";
      }
  ...
  Future<void> loadChartData(String tagId) async {
    if (_documentId == null || _machineId == null || _jobId == null) {
      _syncMessage =
          "ไม่สามารถโหลดกราฟ: Missing documentId, machineId, or jobId.";
      notifyListeners();
      return;
    }
    _isLoading = true;
    ...
    try {
      ...
      print(
          'DocumentRecordViewModel: Stream ข้อมูลกราฟ Local ถูก Initialize แล้วสำหรับ Tag ID: $tagId');
    } catch (e) {
  ...
  Future<void> loadOnlineChartData(String tagId) async {
    if (_documentId == null || _machineId == null || _jobId == null) {
      _syncMessage =
          "ไม่สามารถโหลดกราฟ Online: Missing documentId, machineId, or jobId.";
      notifyListeners();
      return;
    }
    _isLoading = true;
    ...
    try {
      ...
      print(
          'DocumentRecordViewModel: Stream ข้อมูลกราฟ Online ถูก Initialize แล้วสำหรับ Tag ID: $tagId');
    } catch (e) {
  ...
  Future<bool> updateRecordValue(int uid, String? newValue, String? newRemark,
    ...
    try {
      ...
      if (tagType == 'Number' && newValue != null && newValue.isNotEmpty) {
        final String? validationError = _validateNumberInput(
            uid, newValue, jobTag); // ใช้ _validateNumberInput
      ...
      final String? finalUserId =
          userId ?? _loginRepository.loggedInUser?.userId;

      // Determine the status to save based on the input 'newStatus' parameter.
      int statusToSet;
      DbDocumentRecord? currentRecordInDB;
      if (newStatus != null) {
        // ถ้า newStatus ถูกส่งมา (เช่น 0 จากการแก้ไข, 1 จาก Save All Changes)
        statusToSet = newStatus;
      } else {
        // ถ้า newStatus ไม่ได้ถูกส่งมา (หมายถึงการแก้ไขทั่วไปที่ยังไม่รู้สถานะสุดท้าย)
        currentRecordInDB = await _documentRecordRepository.getRecordByUid(uid);
        statusToSet = (currentRecordInDB?.status != 2)
            ? 0
            : 2; // ถ้าไม่ใช่ Status 2 ให้เป็น 0, ถ้าเป็น 2 ให้คงเดิม
      }
  ...
  Future<bool> updateUnReadableStatus(int uid, bool isUnReadable) async {
    ...
    try {
      ...
      String? newValueToSet = isUnReadable
          ? ''
          : currentValue; // ล้างค่าถ้า 'ไม่อ่านค่าได้' เป็น true

      final String? currentUserId = _loginRepository.loggedInUser?.userId;

      final success = await _documentRecordRepository.updateRecordValue(
        // เรียกใช้เมธอด updateRecordValue เดียว
        uid: uid,
        newValue:
            newValueToSet, // ส่งค่าใหม่ (จะถูกล้างถ้า 'ไม่อ่านค่าได้' เป็น true)
        newRemark: currentRemark,
        newUnReadable: newUnReadableStatus, // ส่งสถานะ 'ไม่อ่านค่าได้'
        userId: currentUserId,
        newStatus:
            0, // กำหนดสถานะเป็น 0 (แก้ไข) เพื่อไม่ให้เปลี่ยนแปลงสถานะอื่น
      );
    ...
    try {
      ...
      if (calculatedValue != null) {
        final success = await _documentRecordRepository.updateRecordValue(
          ...
          userId: _loginRepository
              .loggedInUser?.userId, // ส่ง userId เมื่อคำนวณสำเร็จ
        );
        _syncMessage = success
  ...
  Future<bool> _validateAllRecordsForValidation({
    ...
    for (final recordWithTag in currentRecordsSnapshot) {
      ...
      print(
          'Validating record UID ${record.uid}: TagName ${jobTag?.tagName}: Record value "${record.value}", UI value: "$uiValue"');
      ...
      if (jobTag?.tagType == 'User') {
        if (currentUserId != null &&
          ...
          // <<< Changed uiValue.isEmpty to uiValue == '' for clarity if trimmed.
  ...
  Future<bool> uploadAllChangesToServer() async {
    ...
    if (_documentId == null || _machineId == null || _jobId == null) {
      _syncMessage =
          "ไม่สามารถส่งข้อมูลได้: ข้อมูล Document/Machine/Job ID ไม่ครบถ้วน.";
      _statusMessage = "ส่งข้อมูลล้มเหลว.";
      _isLoading = false;
      notifyListeners();
      return false;
